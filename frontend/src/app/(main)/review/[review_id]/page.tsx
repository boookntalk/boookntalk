    ...
    return (
        <div className="w-full h-full overflow-y-auto scrollbar-hide bg-[#F5F5F7] flex flex-col">
            ...
            <Container className="pt-[var(--spacing-1cm,32px)] pb-32 max-w-3xl">
                ...
                <button onClick={() => router.back()} className="flex items-center gap-1.5 text-[13px] font-bold text-gray-400 hover:text-[#0066cc] transition-colors mb-8">
                    <ChevronLeft size={16} /> 이전으로 돌아가기
                </button>

                {/* ▼▼▼ [수정됨] 프리미엄 스퀘어 디자인 적용 (rounded-xl -> rounded-sm) ▼▼▼ */}
                <article className="bg-white rounded-sm shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div onClick={() => router.push(`/work/${review.book.work_id}`)} className="flex items-center gap-4 p-6 bg-gray-50 border-b border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors group">
                        {/* ▼▼▼ [수정됨] 책 커버 사각형 규격 통일 (rounded -> rounded-sm) ▼▼▼ */}
                        <div className="w-12 h-16 relative rounded-sm shadow-sm overflow-hidden bg-white border border-gray-200 shrink-0">
                    ...
                    <div className="p-8 md:p-10 border-b border-gray-100">
                        ...
                        <div className="flex items-center justify-between">
                            <Link href={`/library/${review.author.id}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                                {/* 프로필 이미지는 인물 정보이므로 원형(rounded-full) 유지 */}
