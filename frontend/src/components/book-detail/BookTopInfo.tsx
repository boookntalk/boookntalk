                        ...
                        <div className="relative aspect-[1/1.45] shadow-sm border border-gray-200 rounded-lg overflow-hidden group">
                            <div className="relative w-full h-full transition-transform duration-300 group-hover:scale-[1.02]">
                                {edition.cover_image || edition.cover || work.cover_image ? (
                                    <Image 
                                        src={getHighResCover(edition.cover_image || edition.cover || work.cover_image)} 
                                        alt={work.title} 
                                        fill 
                                        className="object-cover z-10 bg-gray-50"
                                        unoptimized
                                    />
                                ) : (
                        ...
                        <div className="mt-8 bg-[#f5f5f7]/80 rounded-2xl p-4 border border-gray-100 flex flex-wrap items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <div className={`px-3 py-1.5 rounded-lg text-[13px] font-extrabold tracking-wide ${
                                    ...
                                    record.status === 'finished' ? 'bg-[#1d1d1f] text-white' :
                                    'bg-white text-gray-600 border border-gray-200'
                                }`}>
                                    {record.status === 'reading' ? '읽는 중' : record.status === 'finished' ? '완독' : '읽고 싶음'}
                                </div>
