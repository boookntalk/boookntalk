...
export default function BookTopInfo({ 
    ...
    const handleSaveDesc = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/admin/works/${work.id}/description`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            // 업로드 즉시 화면에 임시 반영 (Optimistic UI)
            const previewUrl = URL.createObjectURL(file);
            setCurrentCover(previewUrl);

            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/admin/works/${work.id}/cover`, {
                method: 'POST',
                // FormData 전송 시 Content-Type은 브라우저가 자동 설정하도록 생략해야 합니다!
                body: formData,
            });
                    ...
                    <div className="flex flex-col lg:flex-row gap-[var(--spacing-1cm,32px)] lg:h-[375px]">
                        ...
                        <div className="flex-shrink-0 mx-auto lg:mx-0 w-[200px] lg:w-[220px] flex flex-col h-full pb-8 lg:pb-0">
                            ...
                            <div 
                                ...
                                onClick={() => {
                                    if (!currentCover && isAdmin) fileInputRef.current?.click();
                                }}
                            >
                                <FloatingCover 
                                    src={currentCover}
                                    alt={work?.title || 'Cover'}
                                    className={`w-full h-full ${isUploadingCover ? 'opacity-50' : ''}`}
                                    iconSize={48}
                                />
                                
                                {/* 💡 호버 시 노출되는 업로드 유도 오버레이 */}
                                ...
                                <div className="w-full lg:w-1/3 flex flex-col min-w-0 h-full shrink-0">
                                    {authorInfo && (
                                        <>
                                            ...
                                            {/* 하단 밀착 mt-auto */}
                                            {/* 💡 [수정] authorOtherBooks.length > 0 조건문을 제거하여 영역을 항상 유지합니다. */}
                                            <div className="w-full mt-auto shrink-0 border-t border-gray-100 pt-4">
                                                ...
                                                {/* 💡 [핵심] 데이터가 있을 때와 없을 때를 나누어 렌더링합니다. */}
                                                {authorOtherBooks.length > 0 ? (
                                                    ...
                                                    // 💡 [NEW] 등록된 도서가 없을 때 보여줄 기본 UI (클릭 비활성화, 약간 투명하게 처리)
                                                    <div className="flex items-center gap-3 rounded-xl p-2 -ml-2 border border-transparent opacity-60">
                                                        {/* src에 null을 주어 FloatingCover 내부의 기본 아이콘이 노출되도록 유도합니다. */}
                                                        <FloatingCover src={null} className="w-[44px] shrink-0 aspect-[1/1.45] bg-gray-100" iconSize={12} />
                                                        <div className="flex flex-col flex-1 min-w-0">
                                                            <h4 className="text-[13px] font-bold text-gray-400 line-clamp-2 leading-tight mb-1">
                                                                등록된 대표 도서가 없습니다.
                                                            </h4>
                                                            <span className="text-[11px] font-bold text-gray-300">BoooknTalk</span>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
            ...
            <Dialog open={isDescModalOpen} onOpenChange={setIsDescModalOpen}>
                <DialogContent className="sm:max-w-[600px] bg-white border border-gray-200 rounded-[20px] p-6 shadow-2xl">
                    <DialogHeader className="mb-4">
                        ...
                        <DialogDescription className="text-[13px] text-gray-500 font-medium">
                            {work?.title}의 상세 설명을 수정합니다. 모든 판본에 공통 적용됩니다.
                        </DialogDescription>
                    </DialogHeader>
